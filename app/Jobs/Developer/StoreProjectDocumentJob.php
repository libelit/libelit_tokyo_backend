...
class StoreProjectDocumentJob
{
    protected User $user;
    protected int $projectId;
    protected array $documents;

    public function __construct(User $user, int $projectId, array $documents)
    {
        $this->user = $user;
    ...
    public function handle(): JsonResponse
    {
        try {
            ...
            // Only allow uploading documents to draft or rejected projects
            if (!in_array($project->status, [ProjectStatusEnum::DRAFT, ProjectStatusEnum::REJECTED])) {
                return response()->json([
                    'success' => false,
                    'message' => 'Documents can only be uploaded to draft or rejected projects.',
                ], 403);
            }

            $uploadedDocuments = [];

            DB::transaction(function () use ($project, &$uploadedDocuments) {
                foreach ($this->documents as $docData) {
                    $documentType = $docData['document_type'];
                    $title = $docData['title'];
                    $file = $docData['file'];

                    // Check if document type already exists for this project
                    $existingDocument = $project->documents()
                    ...
                    $filePath = $file->storeAs('documents/project/' . $uuid, $fileName, 'public');
